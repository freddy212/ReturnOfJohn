...
class ShieldCollition(val shield: Shield): MoveCollition{
        ...
        if((collidedObject is Boulder || collidedObject is SmallBoulder)){
            if(collidedObject in delayMap.keys){
                ...
                if(fire == null) {
                    ...
                    val centerBoulder = Vector2(collidedObject.sprite.x + collidedObject.sprite.width / 2, collidedObject.sprite.y + collidedObject.sprite.height)
                    val centerShield = Vector2(shield.sprite.x + shield.sprite.width / 2, shield.sprite.y + shield.sprite.height)
